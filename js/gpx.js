...
// ── Основная функция ───────────────────────────────────────────────────────────
// Три уровня попыток открыть маршрут в Organic Maps:
//   1. Web Share API  — HTTPS + современный браузер → системный шаринг
//   2. Прямой blob URL без download — Android показывает «Открыть с помощью...»
//   3. Скачивание     — универсальный fallback для десктопа
async function exportGpxAndOpen(dayNum) {
  ...
  try {
    ...
    // ── Уровень 1: Web Share API (HTTPS + iOS Safari / Android Chrome) ──────
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      try {
        ...
        console.warn('[gpx] Share API failed:', e);
        // Продолжаем к уровню 2
      }
    }

    // ── Уровень 2: Прямой blob URL (без download) ───────────────────────────
    // Android открывает диалог «Открыть с помощью...» → выбираем Organic Maps
    // iOS Safari скачивает файл и предлагает «Поделиться» → открыть в приложении
    if (isMobile) {
      const blobUrl = URL.createObjectURL(blob);
      try {
        // Создаём ссылку без атрибута download — браузер решает сам, что делать
        const a = document.createElement('a');
        a.href = blobUrl;
        a.type = 'application/gpx+xml';
        // НЕ ставим a.download — это ключевое отличие от fallback
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);

        // Даём время браузеру обработать, потом освобождаем память
        setTimeout(() => URL.revokeObjectURL(blobUrl), 10000);

        showToast(`✅ GPX готов (${stats}) — выберите Organic Maps`);
        return;
      } catch(e) {
        URL.revokeObjectURL(blobUrl);
        console.warn('[gpx] Direct blob open failed:', e);
        // Продолжаем к уровню 3
      }
    }

    // ── Уровень 3: Скачивание + инструкция (десктоп / fallback) ─────────────
