...
var APP_CHANGELOG = [
  { ver: '2.8.0 b64', date: '03.05.2026', items: [
    '📝 Тема заметки — необязательное поле, цвет совпадает с категорией',
    '🎨 Цветовая система категорий: Купить — серый, Взять — песочный, Сделать — фиолетовый, Другое — в цвет темы',
    '🔽 Сворачивание длинных заметок до 5 строк/пунктов — шеврон для разворота',
    '📴 Офлайн-кеш вкладки Заметки (Firebase persistence) — список доступен без интернета',
    '🌤 Погода запрашивается на точную дату маршрута (до 15 дней вперёд)',
    '🌤 Смена даты дня автоматически обновляет погоду',
    '🌤 Фикс: 00:00 больше не трактовался как 12:00',
    '📱 Просмотр фото на мобилке: стрелки перенесены в нижнюю полосу, фото не перекрывается',
    '☁️ Фиксированная ширина статус-бара облака — кнопки не прыгают при смене статуса',
    '❌ Иконка удаления заметки заменена на ❌',
  ]},
